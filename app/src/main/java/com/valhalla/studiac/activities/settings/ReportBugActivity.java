...
public class ReportBugActivity extends NavigationToolbarWhite {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        super.setContent(R.layout.activity_report_bug);

        mInput = findViewById(R.id.report_bug_input_id);
        errorStyle = ErrorStyle.getInstance(getApplicationContext());
        errorStyle.resetError(mInput);

        Button send = findViewById(R.id.report_bug_send_button_id);

        // send an email to domain's account using intent
    ...
    private boolean isInputValid() {
        ...
        if (input.length() <= 10) {
            errorStyle.setError(getString(R.string.Error_DescribeBriefly), mInput);
            return false;
        } else {
            errorStyle.resetError(mInput);
        }
