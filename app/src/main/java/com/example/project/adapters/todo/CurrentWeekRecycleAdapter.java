...
public class CurrentWeekRecycleAdapter extends RecyclerView.Adapter<CurrentWeekRecycleAdapter.CurrentWeekTabViewHolder> {
    ...
    public CurrentWeekRecycleAdapter(ArrayList<TaskItems> taskItems) {
    ...
    public int getItemViewType(int position) {
        ...
        if (mTaskItems.get(position).isSection()) {
            return TYPE_HEADER;
        } else {
            return TYPE_LIST;
        }
    ...
    public void onBindViewHolder(@NonNull CurrentWeekTabViewHolder holder, int position) {
        if (holder.view_type == TYPE_HEADER) {
            holder.mHeaderTextView.setText(mTaskItems.get(position).getCourseName());
        } else {
            TaskItems currentItem = mTaskItems.get(position - 1); // since is one extra row for the header, we are subtracting one
